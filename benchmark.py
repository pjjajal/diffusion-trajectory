...
def diffusion_solve_and_sample(args : argparse.Namespace, solver: CMAES, sample_callable : Callable):
	...
	bench_logger.info("Sampling initial frames")
...
if __name__ == "__main__":
	...
	objective_callable, objective_inner_callable, centroid, solution_length = rotational_transform(
		sample_callable, 
	...
	### Benchmarking!
	###
	sampled_frames_tensor = diffusion_solve_and_sample(args, solver, sample_callable)
	bench_logger.info(f"Sampled frames shape: {sampled_frames_tensor.shape}")

	### Convert `sampled_frames` Tensor to PIL Image list
	### Plot and/or dump frames as necessary
	...
	ffmpeg_make_gif_subprocess(sampled_frames_image_list, frame_dump_path, "animation.gif", fps=2)
