...
def stateful_svd_rot(
    ...
    def _inner_fn(x, update_state = False):
        x = x.reshape(-1, c + c**2)
        # slice out the mean and covariance matrix
        mean = x[:, :c]
        cov = x[:, c:].reshape(-1, c, c)
        ...
        def noise_transform(x):
            x = einsum(x, rot, "b c h w, p c1 c -> p c1 h w")
            x = x + mean_scale * mean
            return x

        # rotate, translate and find the difference between the original and the rotated + translated version.
        samples = sample_fn(noise_transform=noise_transform)
        if update_state:
            sample_fn.update_latents(noise_transform)
        return samples
